...
import { seksbotApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = seksbotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("seksbot-app") as seksbotApp;
...
beforeEach(() => {
  seksbotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SEKSBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  seksbotApp.prototype.connect = originalConnect;
  window.__SEKSBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/seksbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/seksbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/seksbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SEKSBOT_CONTROL_UI_BASE_PATH__ = "/seksbot";
    const app = mountApp("/seksbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/seksbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/seksbot/sessions");
  });
