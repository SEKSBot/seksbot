...
This document describes how seksbot integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

seksbot uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, seksbot directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in seksbot's local TUI mode)                                              |
...
├── pi-tools.ts                    # createseksbotCodingTools()
...
├── seksbot-tools.ts              # seksbot-specific tools
...
2. **Custom Replacements**: seksbot replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **seksbot Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures seksbot's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, seksbot CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
seksbot wraps this with `guardSessionManager()` for tool result safety.
...
seksbot maintains an auth profile store with multiple API keys per provider:
...
seksbot loads custom pi extensions for specialized behavior:
...
seksbot also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | seksbot Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom seksbot tool suite                                                                    |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                  |
| Session storage | `~/.pi/agent/sessions/` | `~/.seksbot/agents/<agentId>/sessions/` (or `$SEKSBOT_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-seksbot-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-seksbot-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-seksbot-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-seksbot-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
