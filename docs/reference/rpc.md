...
seksbot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- seksbot owns lifecycle when `channels.signal.autoStart=true`.
...
- seksbot spawns `imsg rpc` as a child process (legacy iMessage integration).
