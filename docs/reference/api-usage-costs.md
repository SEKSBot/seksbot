...
seksbot features that can generate provider usage or paid API calls.
...
- `seksbot status --usage` and `seksbot channels list` show provider **usage windows**
...
seksbot can pick up credentials from:
...
- `seksbot status --usage`
- `seksbot models status --json`
...
`seksbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
