...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `seksbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `seksbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=seksbot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SEKSBOT_STATE_DIR:-$HOME/.seksbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
