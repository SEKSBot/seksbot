---
summary: "Agent tool surface for seksbot (browser, canvas, nodes, message, cron) replacing legacy `seksbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `seksbot-*` skills
title: "Tools"
---

# Tools (seksbot)

seksbot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `seksbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `seksbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, seksbot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`seksbot node run`).
...
- Requires a Brave API key (recommended: `seksbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated seksbot-managed browser.
...
- Quick smoke: `seksbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `seksbot gateway` restart in-place)
