---
summary: "Chrome extension: let seksbot drive your existing Chrome tab"
read_when:
...
The seksbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate seksbot-managed Chrome profile.
...
seksbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the seksbot release (npm package) as static files. There is no separate “build” step.

After upgrading seksbot:

- Re-run `seksbot browser extension install` to refresh the installed files under your seksbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

seksbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want seksbot to control.
...
- `ON`: attached; seksbot can drive that tab.
...
Debugging: `seksbot sandbox explain`
...
`seksbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `seksbot browser extension install` first to copy the extension to a stable location under your seksbot state directory.
...
- **This is not isolated** like the dedicated seksbot-managed profile.
