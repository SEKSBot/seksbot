...
- **gateway host** → `seksbot` process on the gateway machine
...
`~/.seksbot/exec-approvals.json` directly.

CLI: `seksbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
