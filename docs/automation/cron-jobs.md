...
- Jobs persist under `~/.seksbot/cron/` so restarts don’t lose schedules.
...
seksbot cron list
seksbot cron run <job-id>
seksbot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.seksbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `seksbot cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, seksbot defaults to `announce`.
...
- Job store: `~/.seksbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.seksbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
seksbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
seksbot cron edit <jobId> --agent ops
seksbot cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
seksbot cron run <jobId>
seksbot cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
seksbot cron edit <jobId> \
...
```bash
seksbot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`seksbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SEKSBOT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- seksbot applies exponential retry backoff for recurring jobs after consecutive errors:
