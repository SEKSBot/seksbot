---
summary: "All configuration options for ~/.seksbot/seksbot.json with examples"
read_when:
...
seksbot reads an optional **JSON5** config from `~/.seksbot/seksbot.json` (comments + trailing commas allowed).

If the file is missing, seksbot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.seksbot/workspace`). You usually only need a config to:
...
seksbot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `seksbot doctor`, `seksbot logs`, `seksbot health`, `seksbot status`, `seksbot service`, `seksbot help`).
- Run `seksbot doctor` to see the exact issues.
- Run `seksbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `seksbot config set`. Keep a backup of `~/.seksbot/seksbot.json`.
...
```bash
seksbot gateway call config.get --params '{}' # capture payload.hash
seksbot gateway call config.apply --params '{
...
```bash
seksbot gateway call config.get --params '{}' # capture payload.hash
seksbot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.seksbot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@seksbot", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.seksbot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.seksbot/workspace" }],
}
```
...
{ "$include": "/etc/seksbot/base.json5" } // absolute
...
```json5
// ~/.seksbot/clients/mueller/agents.json5
[
...
```json5
// ~/.seksbot/clients/mueller/broadcast.json5
{
...
seksbot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, seksbot runs your login shell and imports only the missing expected keys (never overrides).
...
- `SEKSBOT_LOAD_SHELL_ENV=1`
- `SEKSBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
seksbot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.seksbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.seksbot/credentials/oauth.json` (or `$SEKSBOT_STATE_DIR/credentials/oauth.json`)
...
- `~/.seksbot/agent/*` (migrated by `seksbot doctor` into `~/.seksbot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `SEKSBOT_OAUTH_DIR`
- Agent dir (default agent root override): `SEKSBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, seksbot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, seksbot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/seksbot/seksbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/seksbot/seksbot.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.seksbot/credentials/whatsapp/biz
          // authDir: "~/.seksbot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `seksbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@seksbot", "seksbot"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@seksbot"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.seksbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.seksbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.seksbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.seksbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.seksbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.seksbot/workspace-home" },
      { id: "work", workspace: "~/.seksbot/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `seksbot.json`).
...
seksbot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["seksbot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-seksbot",
...
seksbot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `seksbot plugins install @seksbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
seksbot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
seksbot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, seksbot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/seksbot" } },
}
```
...
When a file exceeds this limit, seksbot logs a warning and injects a truncated
...
message envelopes). If unset, seksbot uses the host timezone at runtime.
...
`[seksbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[seksbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[seksbot]"`, seksbot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, seksbot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.seksbot/settings/tts.json",
      elevenlabs: {
...
seksbot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, seksbot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `seksbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:seksbot`: all built-in seksbot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.seksbot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.seksbot/sandboxes",
        docker: {
          image: "seksbot-sandbox:bookworm-slim",
          containerPrefix: "seksbot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "seksbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "seksbot-sandbox-browser:bookworm-slim",
          containerPrefix: "seksbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
seksbot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.seksbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
seksbot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, seksbot writes/merges a `models.json` into
`~/.seksbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. seksbot uses
...
- Shortcut: `seksbot onboard --auth-choice opencode-zen`.
...
Shortcut: `seksbot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `seksbot onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `seksbot onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `seksbot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `seksbot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `seksbot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SEKSBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.seksbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.seksbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.seksbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, seksbot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.seksbot/skills` (workspace skills
...
from `~/.seksbot/extensions`, `<workspace>/.seksbot/extensions`, plus any
...
### `browser` (seksbot-managed browser)

seksbot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for seksbot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (seksbot.app menubar, or `seksbot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      seksbot: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/seksbot" }
...
- Examples: `"/ui"`, `"/seksbot"`, `"/apps/seksbot"`.
...
- When a connection comes from one of these IPs, seksbot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `seksbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SEKSBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SEKSBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. seksbot
...
- seksbot.app watches `~/.seksbot/seksbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.seksbot/seksbot.json` (or `SEKSBOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.seksbot/seksbot.json` (or `SEKSBOT_CONFIG_PATH`)
...
- `SEKSBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `seksbot --dev …` → uses `~/.seksbot-dev` + shifts ports from base `19001`
- `seksbot --profile <name> …` → uses `~/.seksbot-<name>` (port via config/env/flags)
...
```bash
SEKSBOT_CONFIG_PATH=~/.seksbot/a.json \
SEKSBOT_STATE_DIR=~/.seksbot-a \
seksbot gateway --port 19001
```
...
Gmail helper config (used by `seksbot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "seksbot@gmail.com",
...
- Set `SEKSBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, seksbot defaults `serve.path` to `/` so
...
Default root: `~/.seksbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the seksbot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__seksbot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__seksbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `SEKSBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.seksbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.seksbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.seksbot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_seksbot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `seksbot` (advertises `seksbot.local`). Override with `SEKSBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_seksbot-gw._tcp` under `~/.seksbot/dns/` using the configured discovery domain (example: `seksbot.internal.`).
