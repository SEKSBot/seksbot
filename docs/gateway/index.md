...
- Replaces the legacy `gateway` command. CLI entry point: `seksbot gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
seksbot gateway --port 18789
# for full debug/trace logs in stdio:
seksbot gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
seksbot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.seksbot/seksbot.json` (or `SEKSBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__seksbot__/canvas/` from `~/.seksbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `SEKSBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `SEKSBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `SEKSBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.seksbot.*` may still exist)
- Linux: `seksbot-gateway-<profile>.service`
- Windows: `seksbot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `SEKSBOT_SERVICE_MARKER=seksbot`
- `SEKSBOT_SERVICE_KIND=gateway`
- `SEKSBOT_SERVICE_VERSION=<version>`
...
```bash
seksbot --dev setup
seksbot --dev gateway --allow-unconfigured
...
- `SEKSBOT_STATE_DIR=~/.seksbot-dev`
- `SEKSBOT_CONFIG_PATH=~/.seksbot-dev/seksbot.json`
- `SEKSBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.seksbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `SEKSBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `SEKSBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `SEKSBOT_CONFIG_PATH`
- unique `SEKSBOT_STATE_DIR`
...
```bash
seksbot --profile main gateway install
seksbot --profile rescue gateway install
```

Example:

```bash
SEKSBOT_CONFIG_PATH=~/.seksbot/a.json SEKSBOT_STATE_DIR=~/.seksbot-a seksbot gateway --port 19001
SEKSBOT_CONFIG_PATH=~/.seksbot/b.json SEKSBOT_STATE_DIR=~/.seksbot-b seksbot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `seksbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `seksbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `seksbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.seksbot.*` is cleaned up).
  - `seksbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are seksbot profile services.
  ...
  - Cleanup: `seksbot gateway uninstall` (current service) and `seksbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `seksbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- seksbot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.seksbot.*` labels still unload cleanly).
- To stop it cleanly, use `seksbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `seksbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `seksbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

seksbot installs a **systemd user service** by default on Linux/WSL2. We
...
`seksbot gateway install` writes the user unit. `seksbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/seksbot-gateway[-<profile>].service`:

```
[Unit]
Description=seksbot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/seksbot gateway --port 18789
Restart=always
RestartSec=5
Environment=SEKSBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now seksbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/seksbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now seksbot-gateway[-<profile>].service
```
...
- `seksbot gateway health|status` — request health/status over the Gateway WS.
- `seksbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `seksbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `seksbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `seksbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `seksbot gateway` and the legacy TCP control port.
