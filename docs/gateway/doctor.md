...
`seksbot doctor` is the repair + migration tool for seksbot. It fixes stale
...
- Extra workspace dir detection (`~/seksbot`).
...
you to run `seksbot doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.seksbot/seksbot.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.seksbot/sessions/` to `~/.seksbot/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.seksbot/agent/` to `~/.seksbot/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.seksbot/credentials/*.json` (except `oauth.json`)
  - to `~/.seksbot/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `seksbot doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.seksbot` folders exist across
  home directories or when `SEKSBOT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.seksbot/seksbot.json` is
...
offers to remove them and install the seksbot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named seksbot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `seksbot doctor --generate-gateway-token` to force token
...
- `seksbot doctor` prompts before rewriting supervisor config.
- `seksbot doctor --yes` accepts the default repair prompts.
- `seksbot doctor --repair` applies recommended fixes without prompts.
- `seksbot doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `seksbot gateway install --force`.
