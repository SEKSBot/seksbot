...
- During heartbeat runs, seksbot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `seksbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, seksbot skips the heartbeat run entirely (no model call).
...
```bash
seksbot system event --text "Check for urgent follow-ups" --mode now
```
