...
read_when:
  - Designing workspace memory (~/.seksbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep seksbot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.seksbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.seksbot/workspace` as canonical human-readable memory.
...
```
~/.seksbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in seksbot**, but keep a separable core library.

### Why integrate into seksbot?

- seksbot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `seksbot memory recall "…" --k 25 --since 30d`
  - `seksbot memory reflect --since 7d`
...
Pragmatic take for `~/.seksbot/workspace`:
