...
read_when:
  - You need detailed behavior for seksbot onboard
...
This page is the full reference for `seksbot onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.seksbot/seksbot.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `seksbot doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `seksbot pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `seksbot health`.
    - `seksbot status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.seksbot/.env` so launchd can read it.
...
- OAuth credentials: `~/.seksbot/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.seksbot/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`seksbot onboard`](/cli/onboard)
