---
summary: "Use MiniMax M2.1 in seksbot"
read_when:
  - You want MiniMax models in seksbot
...
```bash
seksbot plugins enable minimax-portal-auth  # skip if already loaded.
seksbot gateway restart  # restart if gateway is already running
seksbot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/seksbot/seksbot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `seksbot.json`:
...
- Use `seksbot models list` and `seksbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `seksbot configure` and selecting **MiniMax M2.1**, or
