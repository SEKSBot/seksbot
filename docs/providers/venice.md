---
summary: "Use Venice AI privacy-focused models in seksbot"
read_when:
  - You want privacy-focused inference in seksbot
...
```bash
seksbot onboard --auth-choice venice-api-key
```
...
```bash
seksbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, seksbot shows all available Venice models. Pick based on your needs:
...
```bash
seksbot models set venice/claude-opus-45
seksbot models set venice/llama-3.3-70b
```

List all available models:

```bash
seksbot models list | grep venice
```

## Configure via `seksbot configure`

1. Run `seksbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
seksbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
seksbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
seksbot chat --model venice/claude-opus-45

# Use uncensored model
seksbot chat --model venice/venice-uncensored

# Use vision model with image
seksbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
seksbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `seksbot models list` to see currently available models. Some models may be temporarily offline.
