---
read_when:
  - 新規ユーザーにseksbotを紹介するとき
summary: seksbotは、あらゆるOSで動作するAIエージェント向けのマルチチャネルgatewayです。
title: seksbot
x-i18n:
...
<p align="center">
    <img
        src="/assets/seksbot-logo-text-dark.png"
        alt="seksbot"
        width="500"
        class="dark:hidden"
    />
    <img
        src="/assets/seksbot-logo-text.png"
        alt="seksbot"
        width="500"
...
<Columns>
  <Card title="はじめに" href="/start/getting-started" icon="rocket">
    seksbotをインストールし、数分でGatewayを起動できます。
  </Card>
  <Card title="ウィザードを実行" href="/start/wizard" icon="sparkles">
    `seksbot onboard`とペアリングフローによるガイド付きセットアップ。
  </Card>
...
seksbotは、単一のGatewayプロセスを通じてチャットアプリをPiのようなコーディングエージェントに接続します。seksbotアシスタントを駆動し、ローカルまたはリモートのセットアップをサポートします。
...
<p align="center">
  <img src="whatsapp-seksbot.jpg" alt="seksbot" width="420" />
</p>

## 設定（オプション）

設定は`~/.seksbot/seksbot.json`にあります。

- **何もしなければ**、seksbotはバンドルされたPiバイナリをRPCモードで使用し、送信者ごとのセッションを作成します。
...
```json5
{
  ...
  messages: { groupChat: { mentionPatterns: ["@seksbot"] } },
}
```
