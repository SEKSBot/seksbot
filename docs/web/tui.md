...
```bash
seksbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `seksbot tui --deliver`
...
- Check the Gateway logs: `seksbot logs --follow`.
- Confirm the agent can run: `seksbot status` and `seksbot models status`.
...
- No agents in picker: check `seksbot agents list` and your routing config.
