...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/seksbot`)
...
If the page fails to load, start the Gateway first: `seksbot gateway`.
...
you revoke it with `seksbot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.seksbot/seksbot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. seksbot
...
seksbot **blocks** Control UI connections without device identity.
