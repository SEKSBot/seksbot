...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.seksbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.seksbot/node.json` (node id + pairing token).
...
Path: `~/.seksbot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.seksbot/exec-approvals.sock",
...
- Unix socket at `~/.seksbot/exec-approvals.sock` (0600).
