...
- Publish as `seksbot/plugin-sdk` (or export from core under `seksbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `seksbotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: seksbotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: seksbotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: seksbotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: seksbotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: seksbotConfig): string;
  };
};
```
...
- Introduce `seksbot/plugin-sdk`.
- Add `api.runtime` to `seksbotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `seksbotRuntime: ">=2026.2.0"`).
