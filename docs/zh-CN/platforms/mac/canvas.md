...
- `~/Library/Application Support/seksbot/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `seksbot-canvas://<session>/<path>`

示例：

- `seksbot-canvas://main/` → `<canvasRoot>/main/index.html`
- `seksbot-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `seksbot-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
seksbot nodes canvas present --node <id>
seksbot nodes canvas navigate --node <id> --url "/"
seksbot nodes canvas eval --node <id> --js "document.title"
seksbot nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__seksbot__/a2ui/
```
...
seksbot nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
seksbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "seksbot://agent?message=Review%20this%20design";
```
