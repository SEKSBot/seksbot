...
你可以在 `~/.seksbot/seksbot.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/seksbot.log"
  }
}
```
...
```bash
seksbot channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.seksbot/seksbot.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/seksbot/seksbot-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- seksbot 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
SEKSBOT_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `seksbot plugins enable diagnostics-otel` 启用插件。
...
- `seksbot.tokens`（计数器，属性：`seksbot.token`、`seksbot.channel`、`seksbot.provider`、`seksbot.model`）
- `seksbot.cost.usd`（计数器，属性：`seksbot.channel`、`seksbot.provider`、`seksbot.model`）
- `seksbot.run.duration_ms`（直方图，属性：`seksbot.channel`、`seksbot.provider`、`seksbot.model`）
- `seksbot.context.tokens`（直方图，属性：`seksbot.context`、`seksbot.channel`、`seksbot.provider`、`seksbot.model`）

消息流：

- `seksbot.webhook.received`（计数器，属性：`seksbot.channel`、`seksbot.webhook`）
- `seksbot.webhook.error`（计数器，属性：`seksbot.channel`、`seksbot.webhook`）
- `seksbot.webhook.duration_ms`（直方图，属性：`seksbot.channel`、`seksbot.webhook`）
- `seksbot.message.queued`（计数器，属性：`seksbot.channel`、`seksbot.source`）
- `seksbot.message.processed`（计数器，属性：`seksbot.channel`、`seksbot.outcome`）
- `seksbot.message.duration_ms`（直方图，属性：`seksbot.channel`、`seksbot.outcome`）

队列 + 会话：

- `seksbot.queue.lane.enqueue`（计数器，属性：`seksbot.lane`）
- `seksbot.queue.lane.dequeue`（计数器，属性：`seksbot.lane`）
- `seksbot.queue.depth`（直方图，属性：`seksbot.lane` 或 `seksbot.channel=heartbeat`）
- `seksbot.queue.wait_ms`（直方图，属性：`seksbot.lane`）
- `seksbot.session.state`（计数器，属性：`seksbot.state`、`seksbot.reason`）
- `seksbot.session.stuck`（计数器，属性：`seksbot.state`）
- `seksbot.session.stuck_age_ms`（直方图，属性：`seksbot.state`）
- `seksbot.run.attempt`（计数器，属性：`seksbot.attempt`）

### 导出的 span（名称 + 关键属性）

- `seksbot.model.usage`
  - `seksbot.channel`、`seksbot.provider`、`seksbot.model`
  - `seksbot.sessionKey`、`seksbot.sessionId`
  - `seksbot.tokens.*`（input/output/cache_read/cache_write/total）
- `seksbot.webhook.processed`
  - `seksbot.channel`、`seksbot.webhook`、`seksbot.chatId`
- `seksbot.webhook.error`
  - `seksbot.channel`、`seksbot.webhook`、`seksbot.chatId`、`seksbot.error`
- `seksbot.message.processed`
  - `seksbot.channel`、`seksbot.outcome`、`seksbot.chatId`、`seksbot.messageId`、`seksbot.sessionKey`、`seksbot.sessionId`、`seksbot.reason`
- `seksbot.session.stuck`
  - `seksbot.state`、`seksbot.ageMs`、`seksbot.queueDepth`、`seksbot.sessionKey`、`seksbot.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `seksbot doctor`。
