---
summary: "seksbot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up seksbot on a Raspberry Pi
  - Running seksbot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# seksbot on Raspberry Pi

## Goal

Run a persistent, always-on seksbot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most seksbot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled seksbot

# Enable if not
sudo systemctl enable seksbot

# Start on boot
sudo systemctl start seksbot
```
...
```bash
# Check logs
journalctl -u seksbot --no-pager -n 100

# Common fix: rebuild
cd ~/seksbot  # if using hackable install
npm run build
sudo systemctl restart seksbot
```
