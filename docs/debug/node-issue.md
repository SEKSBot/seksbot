...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in seksbot
...
Running seksbot via Node with `tsx` fails at startup with:

```
[seksbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `seksbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch seksbot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node seksbot.mjs status` works on Node 25.
