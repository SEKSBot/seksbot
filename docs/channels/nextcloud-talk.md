...
```bash
seksbot plugins install @seksbot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "seksbot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `seksbot pairing list nextcloud-talk`
  - `seksbot pairing approve nextcloud-talk <CODE>`
