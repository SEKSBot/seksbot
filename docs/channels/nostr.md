...
read_when:
  - You want seksbot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables seksbot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`seksbot onboard`) and `seksbot channels add` list optional channel plugins.
...
```bash
seksbot plugins install @seksbot/nostr
```

Use a local checkout (dev workflows):

```bash
seksbot plugins install --link <path-to-seksbot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "seksbot@example.com",
        "lud16": "seksbot@example.com"
      }
    }
  }
}
