...
```
    ...
    - or read `chat.id` from `seksbot logs --follow`
...
- DM messages can carry `message_thread_id`; seksbot routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    seksbot can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, seksbot retries as plain text.
...
```
    ...
    - `~/.seksbot/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, seksbot enqueues system events like:
...
```bash
seksbot message send --channel telegram --target 123456789 --message "hi"
seksbot message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `seksbot channels status` warns when config expects unmentioned group messages.
    - `seksbot channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `seksbot logs --follow` for skip reasons
