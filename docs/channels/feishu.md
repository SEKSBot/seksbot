...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects seksbot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
seksbot plugins install @seksbot/feishu
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/feishu
```
...
If you just installed seksbot, run the wizard:
...
2. Configuring app credentials in seksbot
...
1. You already ran `seksbot channels add` for Feishu
2. The gateway is running (`seksbot gateway status`)
...
```bash
seksbot pairing approve feishu <CODE>
```
...
2. Run `seksbot logs --follow` and look for `chat_id`
...
2. Run `seksbot logs --follow` and look for `open_id`
...
| `seksbot gateway restart` | Restart gateway service       |
| `seksbot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `seksbot logs --follow`
...
5. Ensure the gateway is running: `seksbot gateway status`
6. Check logs: `seksbot logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.seksbot/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.seksbot/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
