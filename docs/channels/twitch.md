...
read_when:
  - Setting up Twitch chat integration for seksbot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. seksbot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
seksbot plugins install @seksbot/twitch
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SEKSBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "seksbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SEKSBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
SEKSBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
