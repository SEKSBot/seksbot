...
```bash
seksbot pairing list discord
seksbot pairing approve discord <CODE>
```
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in seksbot config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces seksbot auth and returns "not authorized".
...
- If command deploy/state is stale, restart gateway and re-check with `seksbot channels status --probe`.
