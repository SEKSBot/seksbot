...
Matrix is an open, decentralized messaging protocol. seksbot connects as a Matrix **user**
...
```bash
seksbot plugins install @seksbot/matrix
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `seksbot plugins install @seksbot/matrix`
   - From a local checkout: `seksbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: seksbot calls the same
     login endpoint, stores the access token in `~/.seksbot/credentials/matrix/credentials.json`,
...
- On first connection, seksbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  seksbot logs a warning.
...
`~/.seksbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `seksbot pairing list matrix`
  - `seksbot pairing approve matrix <CODE>`
...
- On startup, seksbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
