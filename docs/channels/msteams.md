...
```bash
seksbot plugins install @seksbot/msteams
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
seksbot will offer the local install path automatically.
...
3. Configure seksbot with those credentials.
...
- Talk to seksbot via Teams DMs, group chats, or channels.
...
- On startup, seksbot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.seksbot/seksbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring seksbot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `seksbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `seksbot plugins install @seksbot/msteams`
   - From a local checkout: `seksbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "seksbot in Teams", "full": "seksbot in Teams" },
...
seksbot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, seksbot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/seksbotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

seksbot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `seksbot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.seksbot/msteams-polls.json`.
...
```bash
seksbot message send --channel msteams \
...
seksbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
seksbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
seksbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
seksbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
