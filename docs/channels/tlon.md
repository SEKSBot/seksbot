...
Tlon is a decentralized messenger built on Urbit. seksbot connects to your Urbit ship and can
...
```bash
seksbot plugins install @seksbot/tlon
```

Local checkout (when running from a git repo):

```bash
seksbot plugins install ./extensions/tlon
```
...
Use these with `seksbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, seksbot replies in-thread.
