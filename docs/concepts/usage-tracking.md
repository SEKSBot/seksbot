...
- `/usage cost` in chats: local cost summary aggregated from seksbot session logs.
- CLI: `seksbot status --usage` prints a full per-provider breakdown.
- CLI: `seksbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
