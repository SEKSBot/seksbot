...
read_when:
  - You want to understand what “context” means in seksbot
...
“Context” is **everything seksbot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (seksbot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How seksbot builds the system prompt

The system prompt is **seksbot-owned** and rebuilt each run. It includes:
...
By default, seksbot injects a fixed set of workspace files (if present):
