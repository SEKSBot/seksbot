...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `seksbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".seksbot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SEKSBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SEKSBOT_GATEWAY_TOKEN: inst.gatewayToken,
        SEKSBOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SEKSBOT_GATEWAY_PORT: String(gwA.port),
          SEKSBOT_GATEWAY_TOKEN: gwA.gatewayToken,
          SEKSBOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SEKSBOT_GATEWAY_PORT: String(gwB.port),
          SEKSBOT_GATEWAY_TOKEN: gwB.gatewayToken,
          SEKSBOT_GATEWAY_PASSWORD: "",
        }),
