...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SEKSBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SEKSBOT_STATE_DIR = path.join(base, ".seksbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "seksbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".seksbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
