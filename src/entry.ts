...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.SEKSBOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.SEKSBOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.SEKSBOT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[seksbot] ${parsed.error}`);
    process.exit(2);
  }
