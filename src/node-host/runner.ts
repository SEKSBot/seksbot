...
const execHostEnforced = process.env.SEKSBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SEKSBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureseksbotCliOnPath({ pathEnv: process.env.PATH ?? "" });
