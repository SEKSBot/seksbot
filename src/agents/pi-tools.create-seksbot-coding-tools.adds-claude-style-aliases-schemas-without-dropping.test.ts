...
describe("createseksbotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createseksbotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createseksbotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "seksbot-outside.txt");
