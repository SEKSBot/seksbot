...
function resolveMainSessionKeyForSandbox(params: { cfg?: seksbotConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: seksbotConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`seksbot sandbox explain --session ${runtime.sessionKey}`)}`,
  );
