...
/**
 * Cleanup suspended seksbot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: seksbotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-cli-images-"));
  const paths: string[] = [];
