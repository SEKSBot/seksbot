...
type WebSearchConfig = NonNullable<seksbotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: seksbotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("seksbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.seksbot.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://seksbot.ai",
      "X-Title": "seksbot Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
