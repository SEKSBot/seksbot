...
function resolveExecConfig(cfg: seksbotConfig | undefined) {
...
export function createseksbotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createseksbotReadTool(freshReadTool)];
    }
