...
export function resolveBrokerBaseUrl(provider: string): string | undefined {
  ...
  const cleanBrokerUrl = brokerUrl.replace(/\/$/, ""); // remove trailing slash
 ...
 */
function createBrokerClientIfConfigured(cfg?: seksbotConfig): BrokerClient | undefined {
...
export async function resolveApiKeyForProvider(params: {
  ...
  // At the TOP: check if SEKS broker is configured
  const brokerClient = createBrokerClientIfConfigured(cfg);
  if (brokerClient) {
    try {
      // Get the broker token for this provider
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("seksbot agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
