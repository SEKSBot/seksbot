...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-embedded-agent-"));
...
const ensureModels = (cfg: seksbotConfig) => ensureseksbotModelsJson(cfg, agentDir) as unknown;
