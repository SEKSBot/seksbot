...
import { createseksbotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("seksbot-ws-", async (workspaceDir) => {
      await withTempDir("seksbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createseksbotCodingTools({ workspaceDir });
    ...
    await withTempDir("seksbot-ws-", async (workspaceDir) => {
      await withTempDir("seksbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello seksbot");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("seksbot-ws-", async (workspaceDir) => {
      const tools = createseksbotCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("seksbot-ws-", async (workspaceDir) => {
      await withTempDir("seksbot-override-", async (overrideDir) => {
        const tools = createseksbotCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("seksbot-sandbox-", async (sandboxDir) => {
      await withTempDir("seksbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "seksbot-sbx-test",
        ...
        const tools = createseksbotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
