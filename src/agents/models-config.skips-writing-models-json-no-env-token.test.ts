...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "seksbot-models-" });
}

const MODELS_CONFIG: seksbotConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureseksbotModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureseksbotModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureseksbotModelsJson } = await import("./models-config.js");
      const { resolveseksbotAgentDir } = await import("./agent-paths.js");

      await ensureseksbotModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveseksbotAgentDir(), "models.json");
