...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.SEKSBOT_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-test-"));
    process.env.SEKSBOT_STATE_DIR = stateDir;
    process.env.SEKSBOT_PROFILE = "isolated";

    try {
      const tool = createseksbotTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: seksbot --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/seksbot" } }\n}\n';
