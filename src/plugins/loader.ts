...
import { discoverseksbotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: seksbotPluginDefinition;
  register?: seksbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as seksbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as seksbotPluginDefinition;
...
export function loadseksbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverseksbotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "seksbot/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: seksbotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as seksbotPluginModule;
    } catch (err) {
