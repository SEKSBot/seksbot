...
import { loadseksbotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SEKSBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `seksbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "seksbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SEKSBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadseksbotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SEKSBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadseksbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadseksbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
