...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "seksbot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.seksbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "seksbot-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers SEKSBOT_HOME over HOME", () => {
    vi.stubEnv("SEKSBOT_HOME", "/srv/seksbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/seksbot-home"));
...
describe("shortenHomePath", () => {
  it("uses $SEKSBOT_HOME prefix when SEKSBOT_HOME is set", () => {
    vi.stubEnv("SEKSBOT_HOME", "/srv/seksbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/seksbot-home")}/.seksbot/seksbot.json`)).toBe(
      "$SEKSBOT_HOME/.seksbot/seksbot.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $SEKSBOT_HOME replacement when SEKSBOT_HOME is set", () => {
    vi.stubEnv("SEKSBOT_HOME", "/srv/seksbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/seksbot-home")}/.seksbot/seksbot.json`),
    ).toBe("config: $SEKSBOT_HOME/.seksbot/seksbot.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/seksbot")).toBe(path.resolve(os.homedir(), "seksbot"));
  });
  ...
  it("prefers SEKSBOT_HOME for tilde expansion", () => {
    vi.stubEnv("SEKSBOT_HOME", "/srv/seksbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/seksbot")).toBe(path.resolve("/srv/seksbot-home", "seksbot"));
