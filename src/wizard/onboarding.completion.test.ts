...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "seksbot",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/seksbot.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("seksbot");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "seksbot");
