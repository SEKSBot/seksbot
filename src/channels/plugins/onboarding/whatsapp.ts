...
function setWhatsAppDmPolicy(cfg: seksbotConfig, dmPolicy: DmPolicy): seksbotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: seksbotConfig, allowFrom?: string[]): seksbotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: seksbotConfig, selfChatMode: boolean): seksbotConfig {
...
async function detectWhatsAppLinked(cfg: seksbotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so seksbot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for seksbot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("seksbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
