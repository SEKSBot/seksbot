...
function setSlackDmPolicy(cfg: seksbotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "seksbot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for seksbot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: seksbotConfig, allowFrom: string[]): seksbotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
