...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "seksbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".seksbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<seksbotConfig> = {},
): seksbotConfig {
  const base: seksbotConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "seksbot"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".seksbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
