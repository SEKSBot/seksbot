...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".seksbot-profile-decorated");
}
 ...
 */
export function decorateseksbotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SEKSBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SEKSBOT_BROWSER_COLOR).toUpperCase();
