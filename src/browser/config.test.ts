...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const seksbot = resolveProfile(resolved, "seksbot");
    expect(seksbot?.driver).toBe("seksbot");
    expect(seksbot?.cdpPort).toBe(18800);
    expect(seksbot?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from SEKSBOT_GATEWAY_PORT when unset", () => {
    const prev = process.env.SEKSBOT_GATEWAY_PORT;
    process.env.SEKSBOT_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const seksbot = resolveProfile(resolved, "seksbot");
      expect(seksbot?.cdpPort).toBe(19012);
      expect(seksbot?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.SEKSBOT_GATEWAY_PORT;
      } else {
        process.env.SEKSBOT_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const seksbot = resolveProfile(resolved, "seksbot");
      expect(seksbot?.cdpPort).toBe(19022);
      expect(seksbot?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.SEKSBOT_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "seksbot");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        seksbot: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("seksbot");
  });
});
