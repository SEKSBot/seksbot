import type { BrowserConfig, BrowserProfileConfig, seksbotConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SEKSBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "seksbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SEKSBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SEKSBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an seksbot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for seksbot/seksbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: seksbotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SEKSBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "seksbot";
