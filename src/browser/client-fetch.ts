...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the seksbot gateway (seksbot.app menubar, or \`${formatCliCommand("seksbot gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the seksbot browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the seksbot browser control service. ${hint} (${msg})`);
}
