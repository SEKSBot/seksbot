...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SEKSBOT_GATEWAY_PORT).toBe("18789");
    expect(env.SEKSBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.SEKSBOT_SERVICE_MARKER).toBe("seksbot");
    expect(env.SEKSBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SEKSBOT_SERVICE_VERSION).toBe("string");
    expect(env.SEKSBOT_SYSTEMD_UNIT).toBe("seksbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SEKSBOT_LAUNCHD_LABEL).toBe("ai.seksbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SEKSBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SEKSBOT_SYSTEMD_UNIT).toBe("seksbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SEKSBOT_LAUNCHD_LABEL).toBe("ai.seksbot.work");
    }
  });
});
