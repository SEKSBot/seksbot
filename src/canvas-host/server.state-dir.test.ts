...
describe("canvas host state dir defaults", () => {
  ...
  it("uses SEKSBOT_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("seksbot Canvas");
    } finally {
      await handler.close();
