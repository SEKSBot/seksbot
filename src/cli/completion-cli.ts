...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# seksbot Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(seksbot completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# seksbot Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(seksbot completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $SEKSBOT_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "seksbot") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c seksbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'seksbot gateway' and not 'seksbot other gateway', we need to check the sequence.
