...
${formatHelpExamples([
  ['seksbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['seksbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'seksbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'seksbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['seksbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'seksbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.seksbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.seksbot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['seksbot agents set-identity --agent main --name "seksbot" --emoji "🦞"', "Set name + emoji."],
  ["seksbot agents set-identity --agent main --avatar avatars/seksbot.png", "Set avatar path."],
  [
    "seksbot agents set-identity --workspace ~/.seksbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "seksbot agents set-identity --identity-file ~/.seksbot/workspace/IDENTITY.md --agent main",
