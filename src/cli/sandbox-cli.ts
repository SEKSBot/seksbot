...
const SANDBOX_EXAMPLES = {
  main: [
    ["seksbot sandbox list", "List all sandbox containers."],
    ["seksbot sandbox list --browser", "List only browser containers."],
    ["seksbot sandbox recreate --all", "Recreate all containers."],
    ["seksbot sandbox recreate --session main", "Recreate a specific session."],
    ["seksbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["seksbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["seksbot sandbox list", "List all sandbox containers."],
    ["seksbot sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["seksbot sandbox recreate --all", "Recreate all containers."],
    ["seksbot sandbox recreate --session main", "Recreate a specific session."],
    ["seksbot sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["seksbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["seksbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["seksbot sandbox explain", "Show effective sandbox config."],
    ["seksbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["seksbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["seksbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.seksbot.ai/cli/sandbox")}\n`,
    )
    .action(() => {
