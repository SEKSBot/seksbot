...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      SEKSBOT_NO_RESPAWN: "1",
      SEKSBOT_STATE_DIR: stateDir,
      SEKSBOT_SKIP_CHANNELS: "1",
      SEKSBOT_SKIP_GMAIL_WATCHER: "1",
      SEKSBOT_SKIP_CRON: "1",
      SEKSBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      SEKSBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "seksbot-entry-bootstrap.mjs");
