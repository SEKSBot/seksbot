...
declare const __SEKSBOT_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __SEKSBOT_VERSION__ === "string" && __SEKSBOT_VERSION__) ||
  process.env.SEKSBOT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureseksbotCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[seksbot] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[seksbot] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
