...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.SEKSBOT_CONFIG_PATH;
  const prevDisableCache = process.env.SEKSBOT_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "seksbot-canvas-auth-test-"));
  const configPath = path.join(dir, "seksbot.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.SEKSBOT_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.SEKSBOT_DISABLE_CONFIG_CACHE;
    } else {
      process.env.SEKSBOT_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
