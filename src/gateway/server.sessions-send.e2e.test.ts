...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.SEKSBOT_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.SEKSBOT_GATEWAY_PORT = String(gatewayPort);
  process.env.SEKSBOT_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SEKSBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createseksbotTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
