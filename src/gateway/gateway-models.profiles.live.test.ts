...
import type { seksbotConfig, ModelProviderConfig } from "../config/types.js";
import { resolveseksbotAgentDir } from "../agents/agent-paths.js";
...
import { ensureseksbotModelsJson } from "../agents/models-config.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.SEKSBOT_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SEKSBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SEKSBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SEKSBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<seksbotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.SEKSBOT_CONFIG_PATH,
    token: process.env.SEKSBOT_GATEWAY_TOKEN,
    skipChannels: process.env.SEKSBOT_SKIP_CHANNELS,
    skipGmail: process.env.SEKSBOT_SKIP_GMAIL_WATCHER,
    skipCron: process.env.SEKSBOT_SKIP_CRON,
    skipCanvas: process.env.SEKSBOT_SKIP_CANVAS_HOST,
    agentDir: process.env.SEKSBOT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SEKSBOT_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.SEKSBOT_SKIP_CHANNELS = "1";
  process.env.SEKSBOT_SKIP_GMAIL_WATCHER = "1";
  process.env.SEKSBOT_SKIP_CRON = "1";
  process.env.SEKSBOT_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.SEKSBOT_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveseksbotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-live-state-"));
  process.env.SEKSBOT_STATE_DIR = tempStateDir;
  ...
  process.env.SEKSBOT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.seksbot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveseksbotAgentDir();
  const sanitizedCfg: seksbotConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-live-"));
  const tempConfigPath = path.join(tempDir, "seksbot.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.SEKSBOT_CONFIG_PATH = tempConfigPath;

  await ensureseksbotModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "seksbot live tool probe (local, safe): " +
    ...
    process.env.SEKSBOT_CONFIG_PATH = previous.configPath;
    process.env.SEKSBOT_GATEWAY_TOKEN = previous.token;
    process.env.SEKSBOT_SKIP_CHANNELS = previous.skipChannels;
    process.env.SEKSBOT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SEKSBOT_SKIP_CRON = previous.skipCron;
    process.env.SEKSBOT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SEKSBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SEKSBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureseksbotModelsJson(cfg);
      ...
      const rawModels = process.env.SEKSBOT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.seksbot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
