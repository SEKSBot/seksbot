...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SEKSBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.SEKSBOT_SKIP_GMAIL_WATCHER;
    process.env.SEKSBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SEKSBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SEKSBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SEKSBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
