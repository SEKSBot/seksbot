...
beforeAll(async () => {
  previousToken = process.env.SEKSBOT_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.SEKSBOT_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
