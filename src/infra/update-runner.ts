...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "seksbot.mjs");
    ...
    const doctorStep = await runStep(
      step("seksbot doctor", doctorArgv, gitRoot, { SEKSBOT_UPDATE_IN_PROGRESS: "1" }),
    );
