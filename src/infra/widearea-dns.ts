...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SEKSBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*seksbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "seksbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "seksbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "seksbot-gw");
  ...
  records.push(`_seksbot-gw._tcp IN PTR ${instanceLabel}._seksbot-gw._tcp`);
  records.push(`${instanceLabel}._seksbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._seksbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; seksbot-content-hash: ${contentHash}\n${contentBody}`;
}
