...
export function triggerseksbotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SEKSBOT_SYSTEMD_UNIT,
        process.env.SEKSBOT_PROFILE,
      );
  ...
  const label =
    process.env.SEKSBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SEKSBOT_PROFILE);
