...
function candidateBinDirs(opts: EnsureseksbotPathOpts): string[] {
  ...
  // Bundled macOS app: `seksbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "seksbot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/seksbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "seksbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `seksbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureseksbotCliOnPath(opts: EnsureseksbotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SEKSBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SEKSBOT_PATH_BOOTSTRAPPED = "1";
