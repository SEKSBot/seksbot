...
import { scheduleGatewaySigusr1Restart, triggerseksbotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting seksbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerseksbotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting seksbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
