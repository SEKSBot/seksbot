...
import type { seksbotConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: seksbotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: seksbotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as seksbotConfig),
...
export function getCommandDetection(_cfg?: seksbotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: seksbotConfig) {
