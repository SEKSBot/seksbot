...
import type { seksbotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: seksbotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: seksbotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = seksbotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as seksbotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as seksbotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as seksbotConfig)),
    ),
  };
}
