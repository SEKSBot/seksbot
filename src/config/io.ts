...
import type { seksbotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareseksbotVersions } from "./version.js";
...
function coerceConfig(value: unknown): seksbotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as seksbotConfig;
}
...
function stampConfigVersion(cfg: seksbotConfig): seksbotConfig {
...
function warnIfConfigFromFuture(cfg: seksbotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareseksbotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer seksbot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: seksbotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): seksbotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as seksbotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as seksbotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: seksbotConfig) {
    clearConfigCache();
...
// module scope. `SEKSBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SEKSBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SEKSBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): seksbotConfig {
...
export async function writeConfigFile(cfg: seksbotConfig): Promise<void> {
  clearConfigCache();
