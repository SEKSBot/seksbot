...
const ETC_SEKSBOT_DIR = path.join(ROOT_DIR, "etc", "seksbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "seksbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcseksbotPath(...parts: string[]) {
  return path.join(ETC_SEKSBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcseksbotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "seksbot.json"))).toEqual({
      shared: true,
    });
  });
});
