...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SEKSBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SEKSBOT_STATE_DIR = path.join(home, ".seksbot");
    ...
    await fs.mkdir(path.join(home, ".seksbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.seksbot${path.sep}media`);
