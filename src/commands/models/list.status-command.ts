...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveseksbotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("seksbot models auth setup-token")}\` or \`${formatCliCommand("seksbot configure")}\`.`
          : `Run \`${formatCliCommand("seksbot configure")}\` or set an API key env var.`;
