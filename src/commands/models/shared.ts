...
export async function updateConfig(
  mutator: (cfg: seksbotConfig) => seksbotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: seksbotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: seksbotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("seksbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
