...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveseksbotAgentDir: vi.fn().mockReturnValue("/tmp/seksbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/seksbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/seksbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveseksbotAgentDir: mocks.resolveseksbotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveseksbotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/seksbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/seksbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/seksbot-agent-custom");
