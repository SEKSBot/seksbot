...
import { readConfigFileSnapshot, type seksbotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: seksbotConfig, model: string): seksbotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("seksbot plugins install")}\`.`,
    );
  }
