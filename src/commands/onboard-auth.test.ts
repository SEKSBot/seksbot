...
const requireAgentDir = () => {
  const agentDir = process.env.SEKSBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SEKSBOT_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.SEKSBOT_STATE_DIR;
  const previousAgentDir = process.env.SEKSBOT_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.SEKSBOT_AGENT_DIR;
    } else {
      process.env.SEKSBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SEKSBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SEKSBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-oauth-"));
    process.env.SEKSBOT_STATE_DIR = tempStateDir;
    process.env.SEKSBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SEKSBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SEKSBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-minimax-"));
    process.env.SEKSBOT_STATE_DIR = tempStateDir;
    process.env.SEKSBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SEKSBOT_AGENT_DIR;
