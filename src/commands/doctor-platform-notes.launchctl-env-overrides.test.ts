...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SEKSBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SEKSBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SEKSBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("SEKSBOT_GATEWAY_PASSWORD");
  });
