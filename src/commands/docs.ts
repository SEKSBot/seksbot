...
const SEARCH_TOOL = "https://docs.seksbot.ai/mcp.Searchseksbot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.seksbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('seksbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.seksbot.ai/");
      runtime.log(`Search: ${formatCliCommand('seksbot docs "your query"')}`);
    }
    return;
  }
