...
function resolveMode(cfg: seksbotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("seksbot doctor");

  const root = await resolveseksbotPackageRoot({
  ...
  let cfg: seksbotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("seksbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("seksbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("seksbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("seksbot doctor --fix")}" to apply changes.`);
  }
