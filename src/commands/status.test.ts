...
beforeAll(() => {
  previousProfile = process.env.SEKSBOT_PROFILE;
  process.env.SEKSBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SEKSBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/seksbot-root.js", () => ({
  resolveseksbotPackageRoot: vi.fn().mockResolvedValue("/tmp/seksbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/seksbot/pnpm-lock.yaml",
      markerPath: "/tmp/seksbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("seksbot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("seksbot status --all") ||
          l.includes("seksbot --profile isolated status --all") ||
          l.includes("seksbot status --all") ||
          l.includes("seksbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SEKSBOT_GATEWAY_TOKEN = "abcd1234";
    try {
