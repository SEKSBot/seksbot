...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.SEKSBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "seksbot");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SEKSBOT_STATE_DIR = stateDir;
    process.env.SEKSBOT_CONFIG_PATH = path.join(stateDir, "seksbot.json");
