...
export async function statusCommand(
  ...
  runtime.log(theme.heading("seksbot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("seksbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("seksbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.seksbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.seksbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("seksbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("seksbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("seksbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("seksbot gateway probe")}`);
  }
}
