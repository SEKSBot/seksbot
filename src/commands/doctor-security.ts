...
import type { seksbotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: seksbotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("seksbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("seksbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("seksbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("seksbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("seksbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
