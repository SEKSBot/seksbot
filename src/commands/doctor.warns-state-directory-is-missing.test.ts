...
beforeEach(() => {
  ...
  resolveseksbotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SEKSBOT_STATE_DIR;
  originalUpdateInProgress = process.env.SEKSBOT_UPDATE_IN_PROGRESS;
  process.env.SEKSBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SEKSBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SEKSBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.SEKSBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveseksbotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadseksbotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/seksbot-root.js", () => ({
  resolveseksbotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "seksbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SEKSBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SEKSBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/seksbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SEKSBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
