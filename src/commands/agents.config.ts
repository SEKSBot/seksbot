...
export function listAgentEntries(cfg: seksbotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: seksbotConfig, agentId: string) {
...
function resolveAgentModel(cfg: seksbotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: seksbotConfig): AgentSummary[] {
