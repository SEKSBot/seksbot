...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".seksbot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SEKSBOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SEKSBOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SEKSBOT_STATE_DIR.
