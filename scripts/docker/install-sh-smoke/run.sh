...
INSTALL_URL="${SEKSBOT_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SEKSBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SEKSBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openclaw"
PACKAGE_NAME="${SEKSBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SEKSBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SEKSBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SEKSBOT_INSTALL_LATEST_OUT:-}"
fi
