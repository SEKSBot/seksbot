 ...
 * `seksbot update`. Use it to verify the completion installation flow
...
\`seksbot update\`, \`seksbot doctor\`, and \`seksbot onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# seksbot Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`seksbot completion --install\` later to enable.`));
    return;
  }
