import type { seksbotPluginApi, seksbotPluginService } from "seksbot/plugin-sdk";
...
function normalizeDenyList(cfg: seksbotPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: seksbotPluginApi["config"]): string[] {
...
export default function register(api: seksbotPluginApi) {
  ...
  const timerService: seksbotPluginService = {
