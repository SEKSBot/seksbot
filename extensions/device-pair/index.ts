...
import { approveDevicePairing, listDevicePairing } from "seksbot/plugin-sdk";
...
function resolveGatewayPort(cfg: seksbotPluginApi["config"]): number {
  const envRaw =
    process.env.SEKSBOT_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: seksbotPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: seksbotPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: seksbotPluginApi): Promise<ResolveUrlResult> {
