...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SEKSBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "seksbot-nostr-"));
  process.env.SEKSBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.SEKSBOT_STATE_DIR?.trim() || env.SEKSBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".seksbot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SEKSBOT_STATE_DIR = previous;
    }
