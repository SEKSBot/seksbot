...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["seksbotPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["seksbot", "pollId"]) ??
    readNestedString(value, ["seksbot", "poll", "id"]) ??
    readNestedString(value, ["data", "seksbotPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "seksbot", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          seksbotPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "seksbot poll vote",
          displayText: "Vote recorded",
          value: { seksbotPollId: pollId, pollId },
        },
      },
    ],
  };
