...
vi.mock("seksbot/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("seksbot/plugin-sdk")>("seksbot/plugin-sdk");
  return {
    ...actual,
...
import { emitDiagnosticEvent } from "seksbot/plugin-sdk";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("seksbot.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("seksbot.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("seksbot.webhook.processed");
    expect(spanNames).toContain("seksbot.message.processed");
    expect(spanNames).toContain("seksbot.session.stuck");
