/**
 * Twitch channel plugin for seksbot.
 *
...
import { buildChannelConfigSchema } from "seksbot/plugin-sdk";
import type {
 ...
 * for seksbot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: seksbotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: seksbotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: seksbotConfig): boolean => {
