...
function resolveseksbotRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SEKSBOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SEKSBOT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveseksbotRoot(), "dist", "extensionAPI.js");
