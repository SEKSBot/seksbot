...
import type { seksbotPluginApi, seksbotPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "seksbot-lobster-plugin-") {
...
function fakeApi(overrides: Partial<seksbotPluginApi> = {}): seksbotPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<seksbotPluginToolContext> = {}): seksbotPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: seksbotPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
